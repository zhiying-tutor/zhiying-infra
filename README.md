...
`zhiying-tutor` 项目本地开发与联调用的共享中间件，包含 PostgreSQL 与 RabbitMQ。
...
首次启动会拉取镜像并初始化卷，约 10–30 秒。`docker compose ps` 应显示两个服务都处于 `healthy` 状态。
...
- **端口占用**：检查本机是否已运行其他 PostgreSQL / RabbitMQ 实例，停掉或修改 `compose.yaml` 端口映射。
